...
public class Moteur {
    ...
    void lancerUnePartie() {
        ...
        this.progressionDeLaPartieAkaDeroulementDesTours();
    }
    ...
    void progressionDeLaPartieAkaDeroulementDesTours() {
        int arreterlejeu=0;
        long choix ;
        ...
        while (true) {
            ...
            for (int i=0; i < 8 ; i++) {
                /** on lui propose de choisir entre 1 carte quartier ou 2 pieces
                 * mais on vérifie qu'il y a encore des cartes dans la pioche */
                if (piocheCartesCitadelles.getPiocheCC().size()>0){
                ...
                for (Bot joueur : listeJoueurs) {
                    if(joueur.getPersonnageACeTour().getNumero()==i) {
                        if (choix==1){
                            ...
                            joueur.ajouterCartesCitadellesDansMain(cc.get(choix));
                            cc.remove(0);
                ...
                listeJoueurs.get(i).strategieConstruitDesQuilPeut();    //penser à mettre le if d'après dans god strat pour arreter la partie au bon moment
